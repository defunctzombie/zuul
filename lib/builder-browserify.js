...
function configure(bundler, cfg) {
    ...
    function registerable (cfgObj) {
        forIn(cfgObj, function(value, key) {
    ...
    function register (type, o) {
        ...
        if (type === 'transform' && typeof o[type] === 'object') {
            bundler[type](o[type].name, omit(o[type], 'name'));
        } else {
            bundler[type](o[type], omit(o, type));
        }
    }
}
...
function initBundler(files, config) {
    ...
    var userConfig = find(config.browserify, 'options');
    var browserifyOptions = assign({}, opt);

    if (userConfig) {
        browserifyOptions = assign(browserifyOptions, userConfig.options || {});
    }
