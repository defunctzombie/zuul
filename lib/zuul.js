...
Zuul.prototype.cloudRun = function(done) {
    var self = this;
    var config = self._config;

    if (config.cloud === 'browserstack') {
        self._concurrency = config.browserstack.concurrency ||
            self._concurrency;
    }

    self.registerCloudReporter();

    self.run(function(passed, results) {
        if (passed) {
