...
var currentGitBranch = spawnSync('git', ['rev-parse', '--abbrev-ref', 'HEAD'], { encoding: 'utf8' }).stdout || undefined;
var lastGitCommitShortSha
if (currentGitBranch) {
  currentGitBranch = currentGitBranch.trim();
  lastGitCommitShortSha = spawnSync('git', ['rev-parse', '--short', 'HEAD'], { encoding: 'utf8' }).stdout.trim();
}
...
Zuul.prototype.browser = function(info) {
    var self = this;
    var config = self._config;
    var tags = currentGitBranch && [ currentGitBranch, lastGitCommitShortSha ];
