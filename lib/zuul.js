...
var gitBranch;
var gitCommitShortSha;

exec('git rev-parse --abbrev-ref HEAD && git rev-parse --short HEAD', function(error, stdout, stderr) {
    if (error !== null || stderr) return;
    var tuple = stdout.split(/\n/);
    gitBranch = tuple[0];
    gitCommitShortSha = tuple[1];
});
...
Zuul.prototype.browser = function(info) {
    ...
    if (gitBranch && gitCommitShortSha) {
      conf.tags = [
          gitBranch,
          gitCommitShortSha
      ];
    ...
    self._browsers.push(SauceBrowser(conf, opt));
};
