...
SauceBrowser.prototype.start = function() {
    ...
    self.controller = setup_test_instance(self._opt, function(err, url) {
        ...
        if (opt.sauce_connect) {
            var tunnelId = opt.sauce_connect !== true ? opt.sauce_connect : process.env.TRAVIS_JOB_NUMBER;
            if (tunnelId) init_conf['tunnel-identifier'] = tunnelId;
        }
